...
pub async fn build_mtls_config() -> anyhow::Result<RustlsConfig> {
    ...
    let ca_certs = rustls_pemfile::certs(&mut &ca_cert[..]).collect::<Result<Vec<_>, _>>()?;
    ...
    let client_verify = WebPkiClientVerifier::builder(Arc::new(root_store)).build()?;

    // Load server certificat and key
    let server_cert = tokio::fs::read(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/pki/server/server.crt"
    ))
    .await?;
    let server_key = tokio::fs::read(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/pki/server/server.key"
    ))
    .await?;

    let certs = rustls_pemfile::certs(&mut &server_cert[..]).collect::<Result<Vec<_>, _>>()?;
...
pub async fn mtls_middleware(req: Request, next: Next) -> Response {
    let identity = req
        .extensions()
        .get::<ClientIdentity>()
        .map(|i| i.0.clone())
    ...
    S: Send + 'static,
{
    ...
    fn accept(&self, stream: TcpStream, service: S) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let identity =
                extract_client_cn(&tls_stream).unwrap_or_else(|| "anonymous".to_string());
