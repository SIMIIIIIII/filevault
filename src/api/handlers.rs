...
pub async fn upload_file(
        ...
        .map_err(|e| FileVaultError::TcpSendingError(e.to_string()))?
    {
        ...
        let mut file_disc = fs::File::create(&file_path)
            .await
            ...
            .map_err(|e| FileVaultError::TcpSendingError(e.to_string()))?
        {
            ...
            if total_size > state.max_upload_bytes {
                ...
                return Err(FileVaultError::FileWritingError(
                    "quota dépassé".to_string(),
                ));
            }
            hasher.update(&chunk);
...
pub async fn login(
    ...
    .await
    .map_err(|e| FileVaultError::ConnectionError(e.to_string()))?
    .ok_or(FileVaultError::ConnectionError(
        "identifiants invalides".into(),
    ))?;

    let valid_password = bcrypt::verify(&payload.password, &user.password).unwrap_or(false);
...
pub async fn download_file(
    ...
    let mut file = File_tokio::open(&meta.file_path)
        .await
        .map_err(|e| FileVaultError::FileOpeningError(e.to_string()))?;
    ...
    sqlx::query!(
        "UPDATE files SET nb_downloads = nb_downloads + 1 WHERE id = $1",
        id
    )
    .execute(&state.pool)
    .await
...
pub async fn delete_file(
    ...
    sqlx::query!(
        r#"DELETE FROM files WHERE (id = $1 AND user_id = $2)"#,
        id,
        user_id as i32
    )
    .execute(&state.pool)
