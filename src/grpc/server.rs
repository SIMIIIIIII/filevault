...
use crate::db::{
    check_user, delete_file, get_all_files, get_file, insert_file, new_user, update_nb_download,
};
...
impl FileVault for FileVaultService {
    async fn list(&self, request: Request<ListRequest>) -> Result<Response<ListResponse>, Status> {
        let claims = authenticate_request(&request, &self.jwt_secret).map_err(|error| *error)?;
        ...
        let total =
            i32::try_from(files.len()).map_err(|_| Status::internal("too many files to return"))?;
    ...
    async fn upload(
        ...
        while let Some(chunk) = stream.message().await? {
            match chunk.payload {
                Some(upload_chunk::Payload::Metadata(m)) => {
                    ...
                    let safe_name = Path::new(&m.name)
                        .file_name()
                        .ok_or(Status::internal("Can't créate file"))?
                        .to_string_lossy()
                        .into_owned();

                    let file_path = Path::new("server_files").join(&safe_name);

                    let _ = fs::remove_file(my_file.file_path).await;
                    let file_disc = fs::File::create(&file_path)
                        .await
                        .map_err(|e| Status::internal(e.to_string()))?;
        ...
        let metadata = metadata.ok_or_else(|| Status::data_loss("Metadata manquante"))?;
    ...
    type DownloadStream =
        Pin<Box<dyn tokio_stream::Stream<Item = Result<DataChunk, Status>> + Send>>;
